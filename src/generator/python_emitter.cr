...
module Railcar
  class PythonEmitter
    ...
    def initialize(@indent : Int32 = 0, @program : Crystal::Program? = nil)
    end

    # Check if a method call is a property access (generated by Crystal's `property` macro).
    # If the receiver has a type and that type has an instance var @name, it's a property.
    private def is_property_access?(node : Crystal::Call) : Bool
      ...
      return false unless prog = @program
      return false unless obj_type = obj.type?

      # Check if @name exists as an instance var on the type
      obj_type.instance_vars.has_key?("@#{node.name}")
    rescue
      false
    end
