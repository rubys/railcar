...
module Railcar
  class AppGenerator
    ...
    private def generate_app_entry(app_name : String)
      ...
      io << "  if path == \"/cable\"\n"
      io << "    # Echo Action Cable subprotocol\n"
      io << "    if protocols = context.request.headers[\"Sec-WebSocket-Protocol\"]?\n"
      io << "      protocol = protocols.split(\",\").first.strip\n"
      ...
      io << "    # Serve static files (with path traversal protection)\n"
      io << "    resolved = File.expand_path(\"public\" + path)\n"
      io << "    public_root = File.expand_path(\"public\")\n"
      io << "    if resolved.starts_with?(public_root + \"/\") && File.file?(resolved)\n"
      io << "      context.response.content_type = MIME.from_filename(path, \"application/octet-stream\")\n"
      io << "      context.response.print(File.read(resolved))\n"
      io << "    else\n"
      io << "      begin\n"
