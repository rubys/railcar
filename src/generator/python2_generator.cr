  ...
  class Python2Generator
    ...
    private def emit_helpers(nodes : Array(Crystal::ASTNode), output_dir : String,
      ...
      File.open(helpers_path, "a") do |f|
        ...
        f << "def link_to(text, url, **kwargs):\n"
        f << "    css = kwargs.get('class_') or kwargs.get('class') or ''\n"
        f << "    cls_attr = f' class=\"{css}\"' if css else ''\n"
        f << "    return f'<a href=\"{url}\"{cls_attr}>{text}</a>'\n\n"
        f << "def button_to(text, url, **kwargs):\n"
        f << "    method = kwargs.get('method') or 'post'\n"
        f << "    css = kwargs.get('class_') or kwargs.get('class') or ''\n"
        f << "    form_css = kwargs.get('form_class') or ''\n"
        f << "    data = kwargs.get('data') or {}\n"
        f << "    confirm = data.get('turbo_confirm') or kwargs.get('data_turbo_confirm') or ''\n"
        f << "    cls_attr = f' class=\"{css}\"' if css else ''\n"
        f << "    form_cls = f' class=\"{form_css}\"' if form_css else ''\n"
        f << "    confirm_attr = f' data-turbo-confirm=\"{confirm}\"' if confirm else ''\n"
        f << "    return (f'<form method=\"post\" action=\"{url}\"{form_cls}{confirm_attr}>'\n"
        f << "            f'<input type=\"hidden\" name=\"_method\" value=\"{method}\">'\n"
        f << "            f'<button type=\"submit\"{cls_attr}>{text}</button></form>')\n\n"
